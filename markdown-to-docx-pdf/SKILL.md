...
## Overview
Convert Markdown into a decent DOCX or PDF with a single script. Use `uvx` to run the script with `pypandoc-binary` so Pandoc is available without a system install. For PDF output, the script defaults to `xelatex` with a Chinese-friendly font setup.
...
4. Check the output file and re-run with options (`--toc`, `--metadata`, `--reference-doc`, `--pdf-engine`) if needed.
...
- `--pdf-engine`: Pandoc PDF engine (e.g., `wkhtmltopdf`, `tectonic`). Defaults to `xelatex` for PDF output.
- `--toc`: Include a table of contents.
- `--metadata key=value`: Repeatable metadata entries.

PDF defaults (can be overridden via `--metadata`):
